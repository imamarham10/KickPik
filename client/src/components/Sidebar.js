...
export default function () {
  ...
  return (
    <div
      className={`bg-primary max-h-full pr-7 pl-3 pt-8 relative rounded-tr-full ${
        open ? 'w-40' : 'w-14'
      } duration-1000 flex-col`}
    >
      <ArrowBackRounded
        className={`bg-amber text-primary w-9 h-9 rounded-full  absolute -right-3 top-32 cursor-pointer ${
          !open && 'rotate-180'
        } `}
